...
for i in responseJSON['businesses']:
    newVenue = {}
    newVenue['name'] = i['name']
    if 'price' in i.keys():
        newVenue['price_rating'] = i['price']
    else:
        newVenue['price_rating'] = "(None Given)"
    newVenue['description'] = "(None Given)"
    newVenue['score'] = i['rating']
    allTags = []
    for x in i['categories']:
        allTags.append(x['title'])
    newVenue['tags'] = allTags
    f.write(str(newVenue)+"\n")
f.close()
